...
.\build.ps1 -Version 1.0.9 -ManifestCertificateThumbprint $thumbprint
```
...
统一入口始终构建两个既有 Outlook 配置，不新增 OCR 编译体系：

- `Release-Intranet` 定义 `INTRANET_BUILD`，产出 Local（仅本地 OCR）。
- `Release-Internet` 定义 `INTERNET_BUILD`，产出 LocalOnline（本地 OCR + 网络 OCR）。

两次构建输出分别收集到 `Payload/OutlookLocal` 和 `Payload/OutlookLocalOnline`，不得用一个配置覆盖另一个。

仅构建插件、收集 Payload 和运行静态验证，不触发 WiX 包还原：

```powershell
.\build.ps1 -Version 1.0.9 -SkipInstallerBuild
```
...
Outlook Local 与 Outlook LocalOnline：各自包含主程序集、两个 manifest、VSTO Common/Outlook Utilities，以及运行所需的 Microsoft.Bcl、System.* 与 PdfPig DLL，各 17 个文件。收集脚本还校验两个 `oWorkhelper.dll` 不同，避免误把同一配置复制两次。
